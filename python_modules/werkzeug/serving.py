...
class WSGIRequestHandler(BaseHTTPRequestHandler):
    ...
    def run_wsgi(self) -> None:
        # print("wsgi being run... in thread {}".format(threading.current_thread().ident))
        ...
        def write(data: bytes) -> None:
            ...
            # print("inside write: {}".format(id(status_set)))
        ...
        def start_response(status, headers, exc_info=None):  # type: ignore
            # print("Current headers: {}".format(headers))
...
class ThreadPoolingMixIn: 
    ...
    # lazily create required thread-resources on first time `process_request` !
    thread_pool_size = 2
    thread_pool_created:bool = False
    ...
    def process_request_thread(self):
        ...
        """
        while True:
            # print("[DEBUG]: Handled by: {}".format(threading.current_thread().ident))
    ...
    def process_request(self, request, client_address):
        ...
        if self.thread_pool_created == False:
        ...
        # just put `request` into the common queue!
    ...
    def server_close(self):
        super().server_close()
        print("Yeah whatever, server was closed... but pool may still be active !!!!!")
        # TODO: send some command to running pool to exit!
        
        

# class ThreadedWSGIServer(socketserver.ThreadingMixIn, BaseWSGIServer):
class ThreadedWSGIServer(ThreadPoolingMixIn, BaseWSGIServer):
