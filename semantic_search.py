...
if EXTRA_FEATURES:
    from utils_extra import generate_new_prompt, deduplicate, get_face_embedding, get_frame_types, get_most_similiar_indices_scores, update_blur_scores, get_corrupt_photo_features
...
def scan_dir(dirpath:str) -> List[str]:
    ...
    if os.path.exists(dirpath):
        ...
        return list(filter(lambda x: True if os.path.splitext(x)[1].lower() in ALLOWED_IMAGE_EXTENSIONS else False, os.listdir(dirpath)))
    else:
        return []
...
class ImageIndex(object):
    ...
    def __init__(self, index_path:str = IMAGE_INDEX_DIRECTORY, shard_size:int = SHARD_SIZE, clip_embedding_size:int = CLIP_EMBEDDING_SIZE, face_embedding_size:int = FACE_EMBEDDING_SIZE):
        
        if os.path.exists(IMAGE_INDEX_DIRECTORY) == False:
            if DEBUG:
                print("[DEBUG]: Creating a directory to store IMAGES' indices aka .idx files")
        ...
        if len(temp_shard_files_prefix) > 0:
            curr_shard_suffix = int((sorted(temp_shard_files_prefix, key = lambda x: int(x.strip().split("_")[-1]))[-1]).split("_")[-1])
        ...
        self.current_shard_path = os.path.join(IMAGE_INDEX_DIRECTORY,"{}_{}.idx".format(SHARD_PREFIX, curr_shard_suffix))
        self.current_shard_suffix = curr_shard_suffix

        if curr_shard_idx == 0:
            self.current_shard = np.empty((self.shard_size, self.clip_embedding_size + IMAGE_HASH_SIZE + 1 + (MAX_FACES_COUNT * self.face_embedding_size)), dtype = np.float32) 
        else:
        ...
        if os.path.exists(os.path.join(IMAGE_INDEX_DIRECTORY, "face_database.pkl")):
            with open(os.path.join(IMAGE_INDEX_DIRECTORY,"face_database.pkl"), "rb") as f:
    ...
    def query_face_embeddings(self, face_embeddings, face_ids:List[str]):
        ...
        reference_embeddings = np.concatenate([self.face_database[i].reshape((1,-1)) for i in face_ids], axis = 0) 
        ...
        for i in range(N):
            ...
            for j in range(face_count):
                ...
                t1 = face_embeddings[i:i+1,1 + (j*self.face_embedding_size): 1 + (j+1)*self.face_embedding_size]   
    ...
    def append_embedding(self, image_hash:str, semantic_embedding, face_embeddings = None):
        temp_idx = self.hash2path["CURR_SHARD_IDX"]
        self.current_shard[temp_idx,:self.clip_embedding_size] = semantic_embedding
        self.current_shard[temp_idx,self.clip_embedding_size:(self.clip_embedding_size + IMAGE_HASH_SIZE)] = hash_2_nparray(image_hash)
        ...
        if face_embeddings is not None:
            ...
            for i,embedding in enumerate(face_embeddings):
                self.current_shard[temp_idx, array_ix: array_ix + self.face_embedding_size] = embedding
        ...
        if temp_idx >= self.shard_size:
            ...
            self.current_shard = np.empty((self.shard_size, self.clip_embedding_size + IMAGE_HASH_SIZE + 1 + (MAX_FACES_COUNT * self.face_embedding_size)), dtype = np.float32)
            self.current_shard_path = os.path.join(IMAGE_INDEX_DIRECTORY,"{}_{}.idx".format(SHARD_PREFIX, self.current_shard_suffix))
    ...
    def query(self, semantic_query:Optional[str], shard_suffix:int, shard_idx:int, top_k:int, ids:Optional[List[int]] = None):
        ...
        if shard_suffix == self.current_shard_suffix:
            ...
            with open(os.path.join(IMAGE_INDEX_DIRECTORY,"{}_{}.idx".format(SHARD_PREFIX, shard_suffix)),"rb") as f:
        ...
        if semantic_query is not None:
            ...
            image_hashes = [nparray_2_hash(temp_data[ix, self.clip_embedding_size:(self.clip_embedding_size + IMAGE_HASH_SIZE)]) for ix in semantic_indices] 
        ...
        if ids is not None:
            ...
            face_ids = list(filter(lambda x: True if x in self.face_database else False, ids))
            
            if len(face_ids) > 0:
                
                indices = np.array([i for i in range(temp_data.shape[0])], dtype = np.int64) 
                face_features = temp_data[:,self.clip_embedding_size + IMAGE_HASH_SIZE :][indices]
                temp_output = self.query_face_embeddings(face_embeddings = face_features, face_ids = face_ids)  
                ...
                face_image_hashes = [nparray_2_hash(temp_data[ix, self.clip_embedding_size:(self.clip_embedding_size + IMAGE_HASH_SIZE)]) for ix in face_indices] 
    ...
    def process_dir(self, dirpath:str):
        ...
        for image_file in image_files:
            ...
            if temp_hash is None:
                ...
                if EXTRA_FEATURES:
                    face_features = get_face_embedding(image_data, is_bgr = True, conf_threshold = 0.85, embedding_dim = FACE_EMBEDDING_SIZE)
                    face_features = face_features[:MAX_FACES_COUNT,:]
                else:
                    face_features = None
                image_index.append_embedding(image_hash = temp_hash, semantic_embedding = image_features, face_embeddings = face_features)
                image_curr_count += 1
...
class VideoIndex():
    ...
    def query(self, query:str, video_hash:str, top_k:int,use_augmented_prompt:bool = True, context:int = 1):
        ...
        scores_0 = np.matmul(idx_data[:,:self.embedding_size], text_features.transpose()).ravel()
        if EXTRA_FEATURES:
            indices, scores_0 = get_most_similiar_indices_scores(scores_0, top_k = top_k, context = context)
        else:
        ...
        if EXTRA_FEATURES and use_augmented_prompt:
            scores_1 = np.matmul(idx_data[:,:self.embedding_size], text_features_augmented.transpose()).ravel()
            indices, scores_1 = get_most_similiar_indices_scores(scores_1, top_k = top_k, context = context) 
            ...
            frame_indices = np.concatenate([frame_indices_0, frame_indices_1], axis = 0)[sorted_indices]
            playback_pos = np.concatenate([playback_pos_0, playback_pos_1], axis=0)[sorted_indices]
            new_indices = np.array(deduplicate(list(frame_indices.ravel())), dtype = np.int64)
        else:
            scores = scores_0
    ...
    def process_video(self, index_progress_queue, video_hash:str, video_absolute_path:str, frames_to_skip:int):
        
        
        index_array = np.empty((MAX_FRAMES_INDEXED, self.embedding_size + 3), dtype = np.float32)
        cap = VideoCaptureBasic(video_path = video_absolute_path, frames_to_skip = frames_to_skip)        
        ...
        if FFPROBE_AVAILABLE:
            ...
            if i_frames:
                for frame_no in i_frames:
                    ...
                    image_features = clip.encode_image(frame, is_bgr = False, center_crop = False)
                    index_array[image_features_count,:self.embedding_size] = image_features
                    frame_num = cap.current_frame_idx
                    playback_pos = cap.get_pos_seconds()
                    index_array[image_features_count, self.embedding_size:self.embedding_size + 2] = np.array([frame_num, playback_pos]).astype("float32")
                    
                    image_features_count += 1

                    eta = ((time.time() - tic ) / (image_features_count)) * (len(i_frames) - (image_features_count))
                    eta = str(datetime.timedelta(seconds=eta))
                    index_progress_queue.put((image_features_count / (len(i_frames) + 1e-5), video_hash, eta[:eta.find(".")]))
            ...
            while True:
                ...
                if ret == True:
                    ...
                    eta = ((time.time() - tic ) / image_features_count) * (min(cap.num_frames/frames_to_skip, MAX_FRAMES_INDEXED) - image_features_count)
                    eta = str(datetime.timedelta(seconds=eta))
                    index_progress_queue.put((frame_num / cap.num_frames, video_hash, eta[:eta.find(".")]))
                else:
                    break
        
        if EXTRA_FEATURES:
            update_blur_scores(index_array[:image_features_count,:], self.corrupt_photo_features)

        
        with open(os.path.join(VIDEO_INDEX_DIRECTORY, "{}.idx".format(video_hash)), "wb") as f:
...
def get_video_data(query:str, status_queue, data_queue, client_id, top_k:int = 3, context_window:int = 1):
    ...
    for video_hash in video_hashes:
        ...
        if os.path.exists(video_path):
            ...
            for i,ix in enumerate(frame_indices):
                ...
                img_data = "data:image/jpg;base64, {}".format(base64.b64encode(img_data).decode("utf-8")) 

                
                if data_queue.qsize() == (data_queue.maxsize - 1):
                    print("[DEBUG]: Data queue is full, breaking and aborting this thread. One way to mitigate this to increase the size of queue.")
                    stop_thread  = True
                    break
                data_queue.put((False, frame_scores[i], img_data, "{}_{}".format(video_hash, int(ix))) ) 
        else:
            continue
...
def getIndexCount(media):
    if media.strip() == "video":
        ...
        return flask.jsonify({"index_count": "{}".format(max(0, len(image_index.hash2path) - 1))})
...
def updateDatabase():
    ...
    if EXTRA_FEATURES:
        ...
        for key in image_data_files.keys():
            ...
            img_arr = cv2.imdecode(np.frombuffer(buffer, dtype = np.uint8), cv2.IMREAD_COLOR)  
            del buffer

            if img_arr is not None and img_arr.shape[2] == 3:
                
                face_embedding = get_face_embedding(img_arr, is_bgr = True, conf_threshold = 0.85, embedding_dim = FACE_EMBEDDING_SIZE)
                if face_embedding.shape[0] > 0:
                    
                    image_index.face_database[face_id] = face_embedding[0]
                    
                    with open(os.path.join(IMAGE_INDEX_DIRECTORY,"face_database.pkl"), "wb") as f:
                        pickle.dump(image_index.face_database, f)
                    result["success"] = True
                    result["reason"] = "database updated successfully for face_id: {}".format(face_id)
                else:
                    result["reason"] = "Could not detect any face"
            else:
                result["reason"] = "Could not decode image data, make sure valid colored image."
    else:
    ...
    for i in range(shard_suffix + 1):
        ...
        for score, item in zip(scores, image_hashes):
            ...
            if os.path.exists(image_index.hash2path.get(item)):
                ...
                image_arr = cv2.resize(image_arr, (width_dimension, height_dimension), interpolation=cv2.INTER_LINEAR)
                image_data = cv2.imencode(".jpg", image_arr)[1].tobytes()

                
                
                
                data = "data:image/{};base64, {}".format(imghdr.what(image_index.hash2path[item]), base64.b64encode(image_data).decode("utf-8")) 
                ...
                print("[DEBUG]: could not locate: {}".format(image_index.hash2path[item]))
...
def search(media, top_k:int = 3):
    ...
    if flask.request.form.get("query_start"):
        ...
        if media.strip() == "image":
            temp_thread = Thread(target = get_the_data, args = (text_query, status_queue, data_queue, client_id, top_k))
        else:
            context_window = 1
            if flask.request.form.get("context_window"):
                context_window = int(flask.request.form.get("context_window"))
            temp_thread = Thread(target = get_video_data, args = (text_query, status_queue, data_queue, client_id, top_k, context_window))
...
def get_full_image(local_hash):
    if "_" in local_hash:
        ...
        if temp_path and os.path.exists(temp_path):
            ...
            return flask.Response(image_data, mimetype="image/{}".format(imghdr.what(temp_path)))
        else:
...
def videoIndex(frames_to_skip:int = 10):
    ...
    if video_hash is None:
        return flask.jsonify({"success":False,"reason":"Could not create hash, probably invalid video data or file too short for indexing"})
    ...
    video_index.process_video(temp_queue, video_hash, video_absolute_path = video_absolute_path, frames_to_skip = frames_to_skip)
...
def queryVideo(top_k:int = 3):
    ...
    if video_index.hash2path.get(video_hash, False):
        ...
        scores, frame_indices, playback_pos = video_index.query(query, video_hash, top_k, use_augmented_prompt= aug_prompt)
        ...
        meta_data["data"] = [readFrame(temp_cap, x) for x in list(frame_indices.ravel())]
    else:
        print("[DEBUG]: Could not find video_path for video_hash: {} Probably a bug".format(video_hash))
