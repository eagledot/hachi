var v=Object.defineProperty;var P=(c,e,t)=>e in c?v(c,e,{enumerable:!0,configurable:!0,writable:!0,value:t}):c[e]=t;var n=(c,e,t)=>P(c,typeof e!="symbol"?e+"":e,t);import{f,e as g,h as x,a as w,b as S,c as C,L as I}from"./utils-CWqqr369.js";/* empty css              */import{I as k,P as E,U as F}from"./photoGrid-21K76TKL.js";import{P as z}from"./pagination-TDJLyXTr.js";function L(c){return!c.data_hash||!c.score||!c.meta_data||!(c.data_hash.length===c.score.length&&c.score.length===c.meta_data.length)?(console.error("Malformed rawData in transformRawDataChunk:",c),[]):c.data_hash.map((t,s)=>({id:t,score:c.score[s],metadata:c.meta_data[s]}))}class M{async startSearch(e,t){console.log("Starting search:",e);let s=new FormData;s.append("query_start",String(!0)),s.append("query",e),s.append("page_size",String(t));try{const i=await f(g.IMAGE_SEARCH,{method:"POST",body:s});if(!i.ok)throw new Error(`Image search error: ${i.status}`);return await i.json()}catch(i){throw console.error("Search request failed:",i),i}}async fetchSearchResults(e,t){console.log("Fetching search results for token:",e,"page:",t);try{const s=`${g.COLLECT_QUERY_META}/${e}/${String(t)}`;console.log(s);const i=await f(s);if(!i.ok)throw new Error(`Query results fetch error: ${i.status}`);const r=await i.json();return L(r)}catch(s){throw console.error(s),s}}}const p={person:{icon:"👤",color:"bg-green-100 text-green-800 border-green-200",examples:["john","sarah","mike"],description:"Search for people in your photos",displayName:"People"},query:{icon:"🔍",color:"bg-purple-100 text-purple-800 border-purple-200",examples:["sunset","birthday party","vacation"],description:"Search photo descriptions and content",displayName:"Keywords"},resource_directory:{icon:"📁",color:"bg-yellow-100 text-yellow-800 border-yellow-200",examples:["vacation","photos","2023"],description:"Browse photos by folder or album",displayName:"Folders"}};class A{constructor(){n(this,"currentSuggestionController",null)}buildQueryString(e,t,s){const i={...e};t&&s&&(i[t]=[...i[t]||[],s]);let r="";const o=Object.keys(i).filter(a=>i[a].length>0);for(let a=0;a<o.length;a++){const l=o[a],h=i[l];r+=l+"=";for(let d=0;d<h.length;d++)r+=h[d],d!==h.length-1&&(r+="?");a!==o.length-1&&(r+="&")}return r}getAttributeIcon(e){const t=p[e];return t?t.icon:"#️⃣"}getAttributeColor(e){const t=p[e];return t?t.color:"bg-gray-100 text-gray-800 border-gray-200"}getAttributeDisplayName(e){const t=p[e];return t?t.displayName:e.charAt(0).toUpperCase()+e.slice(1).replace("_"," ")}getAttributeDescription(e){const t=p[e];return t?t.description:"Search attribute"}async getSuggestions(e){const t=new FormData;t.append("query",e);const s=await f(`${g.GET_SUGGESTIONS}`,{method:"POST",body:t});if(s.ok){let i=await s.json();const r=[];let o=Object.keys(i);console.log("Suggestion attributes: ",o);for(let a=0;a<o.length;a++){let l=o[a];const h=Array.from(new Set(i[l]));for(let d=0;d<h.length;d++)r.push({text:h[d],attribute:l,type:"suggestion"})}return r}return[]}getAvailableAttributes(){return Object.keys(p)}}class T{constructor(e,t){n(this,"container");n(this,"fuzzySearchService");n(this,"callbacks");n(this,"filtersContainer");n(this,"inputContainer");n(this,"searchInput");n(this,"searchButton");n(this,"dropdown");n(this,"searchTips");n(this,"selectedFilters",{query:[]});n(this,"suggestions",[]);n(this,"showDropdown",!1);n(this,"selectedIndex",-1);n(this,"hasSearched",!1);n(this,"debounceTimeout",null);console.log("Initializing FuzzySearchUI"),this.container=e,this.callbacks=t,this.fuzzySearchService=new A,this.initializeFilters(),this.createUI(),this.setupEventListeners(),setTimeout(()=>{this.searchInput.focus(),console.log("Search input focused");const{attribute:s,value:i}=this.extractSearchQueryParam();console.log("Extracted query params:",{attribute:s,value:i}),s&&i&&(this.addFilter(s,i),this.executeSearch())},0)}initializeFilters(){this.fuzzySearchService.getAvailableAttributes().forEach(t=>{this.selectedFilters[t]=[]})}extractSearchQueryParam(){const e=new URLSearchParams(window.location.search),t=e.get("person")||"",s=e.get("resource_directory")||"";return t?{attribute:"person",value:t}:s?{attribute:"resource_directory",value:s}:{attribute:null,value:null}}createUI(){this.container.innerHTML=x`
    ...
    `,this.filtersContainer=this.container.querySelector("#filters-container"),this.inputContainer=this.container.querySelector("#input-container"),this.searchInput=this.container.querySelector("#fuzzy-search-input"),this.searchButton=this.container.querySelector("#fuzzy-search-btn"),this.dropdown=this.container.querySelector("#fuzzy-dropdown"),this.searchTips=this.container.querySelector("#search-tips")}handleKeyDown(e){console.log("Key down:",e.key),e.key==="Enter"&&(e.preventDefault(),this.handleAddFilter())}setupEventListeners(){this.searchInput.addEventListener("input",this.handleInputChange.bind(this)),this.searchInput.addEventListener("keydown",this.handleKeyDown.bind(this)),this.searchInput.addEventListener("focus",this.handleInputFocus.bind(this)),this.searchInput.addEventListener("blur",this.handleInputBlur.bind(this)),this.searchButton.addEventListener("click",this.handleSearchClick.bind(this)),document.addEventListener("click",e=>{this.inputContainer.contains(e.target)||this.hideDropdown()})}async handleInputChange(e){const s=e.target.value;console.log("Input changed to:",s),this.selectedIndex=-1,this.debounceTimeout&&clearTimeout(this.debounceTimeout),this.debounceTimeout=setTimeout(async()=>{s.trim().length>0?(this.suggestions=await this.fuzzySearchService.getSuggestions(s),this.suggestions.length>0?(this.showDropdown=!0,this.renderDropdown()):this.hideDropdown()):(this.suggestions=[],this.hideDropdown())},300)}async handleInputFocus(){this.selectedIndex=-1,this.searchInput.value.trim()&&(this.suggestions=await this.fuzzySearchService.getSuggestions(this.searchInput.value),this.suggestions.length>0?(this.showDropdown=!0,this.renderDropdown()):this.hideDropdown())}handleInputBlur(){setTimeout(()=>{this.suggestions=[],this.selectedIndex=-1,this.hideDropdown()},200)}async handleAddFilter(){var e;if(this.searchInput.value.trim()){const t=this.searchInput.value.trim();(e=this.selectedFilters.query)!=null&&e.includes(t)||this.addFilter("query",t),this.searchInput.value="",this.suggestions=[],this.hideDropdown(),this.executeSearch()}}async handleSuggestionClick(e){this.addFilter(e.attribute,e.text),this.searchInput.value="",this.suggestions=[],this.hideDropdown(),this.executeSearch()}async handleSearchClick(){this.searchInput.value.trim()?await this.handleAddFilter():this.executeSearch()}addFilter(e,t){this.selectedFilters[e]||(this.selectedFilters[e]=[]),this.selectedFilters[e].includes(t)||(e==="query"||e==="resource_directory"?this.selectedFilters[e]=[t]:this.selectedFilters[e].push(t),this.renderFilters())}removeFilter(e,t){if(this.selectedFilters[e]){this.selectedFilters[e]=this.selectedFilters[e].filter(r=>r!==t),this.renderFilters();const s=Object.keys(this.selectedFilters).some(r=>this.selectedFilters[r]&&this.selectedFilters[r].length>0),i=this.searchInput.value.trim().length>0;(s||i)&&this.executeSearch()}}executeSearch(){const e=this.fuzzySearchService.buildQueryString(this.selectedFilters);console.log("Executing search with query:",e),this.hasSearched||(this.hasSearched=!0,this.hideSearchTips()),localStorage.setItem("lastSearchQuery",e),this.callbacks.onSearchExecuted(e)}renderFilters(){const e=Object.keys(this.selectedFilters).filter(s=>this.selectedFilters[s].length>0).map(s=>this.selectedFilters[s].map(i=>{const r=this.fuzzySearchService.getAttributeIcon(s);return`
          ...
          `}).join("")).flat().join("");this.filtersContainer.innerHTML=e,e.length>0?this.filtersContainer.classList.remove("invisible"):this.filtersContainer.classList.add("invisible"),this.filtersContainer.querySelectorAll(".remove-filter-btn").forEach(s=>{s.addEventListener("click",i=>{i.stopPropagation();const r=s.getAttribute("data-attribute"),o=s.getAttribute("data-value");this.removeFilter(r,o)})})}renderDropdown(){if(!this.showDropdown){this.hideDropdown();return}const e=this.dropdown.querySelector("#dropdown-content");if(this.suggestions.length>0){const t=this.suggestions.reduce((o,a)=>(o[a.attribute]||(o[a.attribute]=[]),o[a.attribute].push(a),o),{});let s="",i=0;Object.keys(t).forEach(o=>{const a=t[o],l=this.fuzzySearchService.getAttributeIcon(o),h=this.fuzzySearchService.getAttributeDisplayName(o);Object.keys(t).length>1&&(s+=`
            <div class="px-3 sm:px-4 py-2 sm:py-3 text-xs sm:text-sm font-semibold text-gray-600 bg-gray-50 border-b border-gray-100 flex items-center">
              <span class="mr-2 sm:mr-3">${l}</span>
              ${h}
            </div>
          `),a.forEach(d=>{const u=this.fuzzySearchService.getAttributeColor(d.attribute);s+=`
            <div class="suggestion-option flex items-center px-2 sm:px-3 py-1.5 cursor-pointer border-b border-gray-100 last:border-b-0 group hover:bg-blue-50 transition ${this.selectedIndex===i?"bg-blue-50 border-l-2 border-l-blue-500":""}" data-index="${i}">
              <div class="flex items-center justify-center w-7 sm:w-8 h-7 sm:h-8 rounded-lg mr-2 sm:mr-3 ${u}">
                <span class="text-xs sm:text-sm">${l}</span>
              </div>
