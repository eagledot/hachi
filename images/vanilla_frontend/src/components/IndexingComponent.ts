...
export class IndexingComponent {
    ...
    private saveToLocalStorage() {
        try {
            localStorage.setItem('stored_indexing_endpoint', JSON.stringify(this.currentStatusEndpoint));
        } catch (e) {
    ...
    private render() {
        ...
        card.innerHTML = html`
            ...
            ${this.error ? `<div class="bg-red-50 border border-red-200 rounded-lg p-4 mb-6">
                <div class="flex">
            ...
            <div class="space-y-4">
                ...
                <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
                    <div class="flex justify-between items-center mb-2">
                        ...
                        <span class="text-sm font-semibold text-blue-600">${(this.indexProgress * 100).toFixed(1)}%</span>
                    </div>
                    <div class="w-full bg-gray-200 rounded-full h-2 mb-3">
                        <div class="bg-blue-600 h-2 rounded-full transition-all duration-300" style="width: ${this.indexProgress * 100}%"></div>
                    </div>
                    <div class="space-y-1 text-sm text-gray-600">
                        ${this.directoryBeingIndexed ? `<div><span class="font-medium">Scanning:</span> ${this.directoryBeingIndexed}</div>` : ''}
                        ${this.eta ? `<div><span class="font-medium">Time left:</span> ${this.eta}</div>` : ''}
                        ${this.extraDetails ? `<div><span class="font-medium">Status:</span> ${this.extraDetails}</div>` : ''}
                    </div>
                </div>
                ` : ''}
                ...
                <div class="flex items-start space-x-3">
                    ...
                    />
                    <div>
                        <label for="complete-rescan" class="text-sm font-medium text-gray-700 cursor-pointer">
                            Re-scan All Photos
                        </label>
                        <p class="text-xs text-gray-500 mt-1 leading-relaxed">
                            Scans all photos again, even if already added. Use this if photos were missed, changed, or for a more thorough update.
                        </p>
                    </div>
                </div>
                ...
                <div class="flex space-x-3 pt-4">
                    <button 
                        id="scan-btn" 
                        ${this.isIndexing || !this.indexDirectoryPath.trim() ? 'disabled' : ''} 
    ...
    private attachEvents() {
        ...
        const protocolSelect = this.root.querySelector<HTMLSelectElement>('#protocol-select');
        if (protocolSelect) {
        ...
        const rescanCheckbox = this.root.querySelector<HTMLInputElement>('#complete-rescan');
        if (rescanCheckbox) {
            rescanCheckbox.addEventListener('change', (e) => {
                this.completeRescan = (e.target as HTMLInputElement).checked ? 'true' : 'false';
            });
        }
    ...
    private async startIndexing() {
        ...
        this.error = null;        this.isIndexing = true;
        this.isCancelling = false;
        this.showNotification('Preparing to scan...', 'info');
        this.updateErrorSection();
        this.updateProgressSection();
        this.updateActionButtons();        this.updateInputsState();
        ...
        try {
            ...
            if (data.success && data.statusEndpoint) {
                ...
                this.pollIndexStatus();
            } else {                this.error = data.reason || 'Failed to start scanning photos.';
        ...
        } catch (e) {            this.error = 'Could not start scanning. Please check the folder path or service.';
            this.isIndexing = false;
            this.showNotification(this.error, 'error');
            this.updateErrorSection();
            this.updateProgressSection();
    ...
    private async pollIndexStatus() {
        ...
        try {
            const resp = await fetch(`${this.apiUrl}/getIndexStatus/${this.currentStatusEndpoint}`);
            const data: IndexStatusResponse = await resp.json();
            if (data.done) {
                // Send ack
                await fetch(`${this.apiUrl}/getIndexStatus/${this.currentStatusEndpoint}`, {
                    method: 'POST',
                    body: (() => { const f = new FormData(); f.append('ack', 'true'); return f; })()
                });
                this.isIndexing = false;
                this.currentStatusEndpoint = null;
                this.saveToLocalStorage();
                ...
                this.extraDetails = '';
                this.eta = '';                this.showNotification(`Scan complete! ${data.details}`, 'success');
                this.updateProgressSection();
                this.updateActionButtons();
                this.updateInputsState();
    ...
    private async cancelIndexing() {
        ...
        try {
            ...
            this.isIndexing = false;
            this.currentStatusEndpoint = null;
            this.saveToLocalStorage();            this.showNotification('Scan stopped successfully', 'warning');
            this.updateProgressSection();
            this.updateActionButtons();
            this.updateInputsState();
        } catch (e) {            this.error = 'Could not stop the scan. Please contact administrator.';
            this.isCancelling = false;
            this.showNotification(this.error, 'error');
            this.updateErrorSection();
            this.updateActionButtons();
