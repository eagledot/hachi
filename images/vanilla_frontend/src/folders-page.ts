...
class FoldersApp {
  ...
  private renderGridViewCard(folder: DirectoryWithPreview): string {
    const previewImage = folder.previewImage
      ? `<img src="${folder.previewImage}" alt="${folder.name}" class="w-full h-56 object-cover">`
    ...
    return `
      <div class="folder-card bg-white rounded-xl shadow-md overflow-hidden border border-gray-200 hover:shadow-xl transition-all duration-300 cursor-pointer group relative" 
        ...
        <div class="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent p-3">
          <h3 class="font-semibold text-white text-sm leading-tight mb-1 line-clamp-2" title="${folder.name}">
            ${folder.name}
          </h3>
        ...
        <div class="absolute inset-0 bg-black/0 group-hover:bg-black/5 transition-all duration-300 flex items-center justify-center">
          <div class="opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-white/90 backdrop-blur-sm rounded-full p-2">
