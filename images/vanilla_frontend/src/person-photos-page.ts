...
class PersonPhotosApp {
  ...
  private updatePersonInfo(): void {
    ...
    if (avatar) {
      ...
      avatar.onerror = () => {
        avatar.src = "./assets/sample_place_bg.jpg";
      };
    }
    ...
    if (nameElement) {
      ...
      const displayName =
        this.personId.charAt(0).toUpperCase() +
        this.personId.slice(1).toLowerCase();
    ...
    if (photoCountElement) {
      const photoCount = this.personPhotosData.data_hash
        ? this.personPhotosData.data_hash.length
        : 0;
  ...
  private renderPhotos(): void {
    ...
    if (
      !this.personPhotosData.data_hash ||
      this.personPhotosData.data_hash.length === 0
    ) {
      photoGrid.innerHTML = "";
    ...
    const photosForGrid: HachiImageData[] = this.personPhotosData.data_hash.map(
      (hash, index) => ({
        id: hash,
  ...
  private async savePersonName(): Promise<void> {
    ...
    try {
      ...
      if (result.success) {
        ...
        this.showError(
          result.reason || "Failed to save name. Please try again."
        );
