<script>
  import logo_name from '../assets/logo_name.min.svg'
  import logo from '../assets/logo.min.svg'
  import folder_plus from '../assets/folder-plus.svg'
  ...
  async function getFolder() {
    const url = url_prefix + "/select_dir";
    ...
    dirname = result['video_directory'];
    localStorage.setItem('dirname', dirname)
  }
  
  function videoIndexSearch() {
    window.location.href = "/video_search.html";
  }

  function imageIndexSearch() {
    window.location.href = "/search.html";
  }

  const options = [
    // {id:1, func: getFolder, text: 'select folder', svg: folder_plus},
    {id:2, func: videoIndexSearch, text: 'Video Search', svg: fire},
    {id:3, func: imageIndexSearch, text: 'Image Search', svg: fire},
    // {id:4, func: indexAll, text: 'index all'},
    // {id:5, func: indexAll, text: 'index all'},
    // {id:6, func: indexAll, text: 'index all'},
  ];
</script>
<!-- Side bar -->
<nav class="md:w-56 max-md:flex max-md:flex-col bg-gray-100 dark:bg-gray-500 md:overflow-y-auto py-4">
  <div>
    <a class="flex md:flex-col items-center" href="https:ramanlabs.in">
      <!-- ramanlabs logo -->
      <svg class="h-20 w-40 m-4 md:mt-10" viewBox="0 0 110 110">
        <circle class="fill-[#00f] opacity-[.24797] dark:fill-pink-100 dark:opacity-100" cx="55" cy="55" r="55" style="stroke-width:3.1181"/>
        <path style="fill-opacity:.62308;fill:#1a1a1a;stroke-width:4;stroke:#000" d="M41.451 49.81h26.458v26.458H41.451z"/>
        ...
        <circle cx="46.743" cy="55.1" r=".671" style="fill-opacity:.62308;fill:#1a1a1a;stroke-width:2;stroke:#000"/>
      </svg>
      <!-- ramanlabs name logo -->
      <svg class="h-8 w-40 m-4" viewBox="0 0 57.975 8.308">
        <g style="stroke-width:.26458">
  ...
  <ul class="">
    {#each options as option(option.id)}
      <li class="font-semibold text-gray-700 dark:text-white hover:bg-pink-200 hover:dark:text-gray-700 mb-2 transition duration-500">
        <button on:click={option.func} class="w-full pl-8 py-2">
          <span class="capitalize flex items-center">
            <!-- add folder svg -->
            {#if option.svg}
              <img src={option.svg} alt={option.text} class="dark:text-white w-6 h-6">
            {/if}
