...
class VideoCaptureBasic(object):
    ...
    def read(self, bgr = True):
        ...
        """
        frame = self.read_specific_frame(frame_num = self.current_frame_idx + self.frames_to_skip + 1, bgr = bgr)
...
def get_frame_variance(frame, pixel_count:int = 100) -> float:
    ...
    temp_std = []
    for i in range(7):
        temp_std.append( np.std([np.sum((frame.reshape((-1))[(x*C) : (x+1)*C]).astype(np.float32)) for x in np.random.randint(low = 0, high = H*W, size = pixel_count)]))
