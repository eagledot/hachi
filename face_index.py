...
import numpy as np
def compare_face_embeddings(self, query:np.array, data_embeddings:np.array, embedding_size:int = 512) -> tuple[np.array, np.array]:
        ...
        simiilarity_scores = np.square(query.reshape((1, self.embedding_size)) - data_embeddings).sum(axis = -1)
