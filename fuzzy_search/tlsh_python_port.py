...
"""
TLSH pure python port based on the `github.com/trendmicro/tlsh` implementation.

    Author:     eagledot (Anubhav N.)
    Copyright:  (c) Anubhav N. 2023
    License:    Apache 2.0 or BSD (based on the original license for github.com/trendmicro/tlsh).
...
def swap_byte(i:int):
    assert i >=0 and i <= 255, "i is {}".format(i)         # [0-255 uint8 data]
    byte = 0
    byte = ((i & 0xf0) >> 4) & 0x0F
    byte |= ((i & 0x0f) << 4) & 0xF0
    return byte
...
def from_hex(data:str) -> bytearray:
...
MIN_DATA_LENGTH = 40    # in bytes, default was 256. but i am testing with 50.

class Uint32Array(object):
    # for now modelling it with standard int, if doesn't work, we can use numpy.
...
class Tlsh(object):
    def __init__(self) -> None:
        ...
        self.bit_pairs_diff_table = bit_pairs_diff_table  # using precacluated table, or generateTable()

    def update(self, data:Union[str, bytes], length = None):
        self.lsh_code_valid = False

        if isinstance(data, str):
            data_array = bytearray(data.encode("utf8"))
        else:
    ...
    def final(self, data = None, length = None):
        ...
        assert self.data_len >= MIN_DATA_LENGTH, "Got: {} expected: {} Length too small..".format(self.data_len, MIN_DATA_LENGTH)

        self.find_quartiles() # find and update quartiles for provided data.
        ...
        if (nonzero <= 4 * (CODE_SIZE // 2)):
            FLAG = False
            print("[WARNING: ] not enough variation..")
        ...
        tmp_q3 = max(1, self.q3)
        self.Q = setQLo(self.Q, int (((self.q1 * 100)/ tmp_q3) %16 ))
        self.Q = setQHi(self.Q, int(((self.q2 * 100)/ tmp_q3) %16 ))
        self.lsh_code_valid = True
        return FLAG
    
    def digest(self) -> bytes:
        # flag if True, indicates a usable HASH, otherwise not enough VARIATION detected to produce a USABLE HASH
        flag = self.final()
        self.hash()
        return flag, bytes(from_hex(self.lsh_code))
    
    def hexdigest(self) -> str:
        # flag if True, indicates a usable HASH, otherwise not enough VARIATION detected to produce a USABLE HASH
        flag = self.final()
        self.hash()
        return flag, self.lsh_code
    ...
    def totalDiff(self, x, len_diff:bool = False):  # how to set x type to Tlsh?
        if self == x:
            return 0
        
        # make sure final has been called upon.
        if self.lsh_code_valid == False:
            self.final()
        if x.lsh_code_valid == False:
            x.final()
        ...
        if (len_diff):
            ldiff = mod_diff(self.Lvalue, x.Lvalue, RANGE_LVALUE)
            if (ldiff == 0):
                diff = 0
            elif (ldiff == 1):
                diff = 1
        ...
        q1diff = mod_diff( getQLo(self.Q) , getQLo(x.Q), RANGE_QRATIO)
        if (q1diff <= 1):
            diff += q1diff
        else:
            diff += (q1diff - 1)*12
        
        q2diff = mod_diff( getQHi(self.Q), getQHi(x.Q), RANGE_QRATIO)
        if (q2diff <=1 ):
            diff += q2diff
        else:
            diff += (q2diff - 1)*12
        ...
        for k in range(0, TLSH_CHECKSUM_LEN):
            if (self.checksum[k] != x.checksum[k]):
                diff += 1
                break
        
        diff += h_distance(self.bit_pairs_diff_table, CODE_SIZE, self.tmp_code, x.tmp_code)
...
if __name__ == "__main__":
    m = Tlsh()
    m.update("thi si suppoed to be a test abcdghijk to try and find the enough variation in the first place")
    flag, hash = m.hexdigest()      # hex-encoded hash (to be easily passed as string)     length = 70
    print(hash)
    flag, hash = m.digest()         # bytes encoded hash  , length = 35
    print(hash)
    print(list(bytearray(hash)))
